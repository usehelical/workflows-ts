...
import { createPgLiteDriver } from '@internal/db/driver-pglite';
...
export async function setupTestDatabase(): Promise<void> {
  ...
  const migrationPath = join(__dirname, '../internal/db/migrations/init.up.sql');
...
vi.mock('@internal/db/driver-pg', async (importOriginal) => {
  const actual = await importOriginal<typeof import('@internal/db/driver-pg')>();
  return {
    ...actual,
